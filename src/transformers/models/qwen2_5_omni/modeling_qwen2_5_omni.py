...
class MyLinear(nn.Module):
    ...
    def forward(self, input: Tensor, task_ids: Optional[Tensor] = None) -> Tensor:
...
class Qwen2_5OmniAudioAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        query_states = self.q_proj(hidden_states, task_ids=task_ids).reshape(
            seq_length, self.num_heads, -1
        )
        key_states = self.k_proj(hidden_states, task_ids=task_ids).reshape(
            seq_length, self.num_heads, -1
        )
        value_states = self.v_proj(hidden_states, task_ids=task_ids).reshape(
            seq_length, self.num_heads, -1
        )
...
class Qwen2_5OmniAudioEncoderLayer(GradientCheckpointingLayer):
    ...
    def forward(
        ...
        **kwargs,
        ...
        hidden_states = self.fc1(hidden_states, task_ids=task_ids)
        hidden_states = self.activation_fn(hidden_states)
        hidden_states = self.fc2(hidden_states, task_ids=task_ids)
        hidden_states = residual + hidden_states
        ...
        **kwargs,
        ...
        for i, each_audio_states in enumerate(hidden_states_list):
            ...
            if os.environ.get("GET_FEATURE", "0") == "1":
                ...
                with open(
                    f"features/{task_ids[i]}/{original_hashs[i]}/metadata.txt",
                    "a",
                    encoding="utf-8",
                ) as f:
                    f.write("===========audio.pkl=================\n")
                    ...
                    for key in output_dict["features"].keys():
                        f.write(
                            f"{key} {type(output_dict['features'][key])} {output_dict['features'][key].shape}\n"
                        )
                    f.write("\n")
                    for key in output_dict.keys():
                        if key != "features":
                            f.write(
                                f"{key} {type(output_dict[key])} {output_dict[key].shape}\n"
                            )
...
class Qwen2_5OmniVisionAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        query_states = self.q(hidden_states, task_ids=task_ids).reshape(
            seq_length, self.num_heads, -1
        )
        key_states = self.k(hidden_states, task_ids=task_ids).reshape(
            seq_length, self.num_heads, -1
        )
        value_states = self.v(hidden_states, task_ids=task_ids).reshape(
            seq_length, self.num_heads, -1
        )
...
class Qwen2_5OmniMLP(nn.Module):
    ...
    def forward(
        self, hidden_state, task_ids: Optional[torch.Tensor] = None
    ) -> torch.Tensor:
        return self.down_proj(
            self.act_fn(self.gate_proj(hidden_state, task_ids=task_ids))
            * self.up_proj(hidden_state, task_ids=task_ids),
            task_ids=task_ids,
        )
    ...
    def forward(
        ...
        **kwargs,
        ...
        hidden_states = hidden_states + self.mlp(
            self.norm2(hidden_states), task_ids=task_ids
        )
        return hidden_states
...
class Qwen2_5OmniPatchMerger(nn.Module):
    ...
    def forward(
        ...
        x = self.mlp[0](x, task_ids=task_ids)
        x = self.mlp[1](x)
        x = self.mlp[2](x, task_ids=task_ids)
        return x
...
class Qwen2_5OmniAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        query_states = self.q_proj(hidden_states, task_ids=task_ids)
        key_states = self.k_proj(hidden_states, task_ids=task_ids)
        value_states = self.v_proj(hidden_states, task_ids=task_ids)
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        hidden_states = self.mlp(
            hidden_states, task_ids=kwargs.get("task_ids", None)
        )  # BC for task_ids
        hidden_states = residual + hidden_states
...
class Qwen2_5OmniThinkerTextModel(Qwen2_5OmniPreTrainedModel):
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        if os.environ.get("GET_FEATURE", "0") == "1":
            output_dict = [{} for i in range(inputs_embeds.shape[0])]
        ...
        if os.environ.get("GET_FEATURE", "0") == "1":
            for j in range(inputs_embeds.shape[0]):
                ...
                with open(
                    f"features/{task_ids[j]}/{original_hashs[j]}/llmfeature.pkl",
                    "wb",
                ) as f:
                    pickle.dump(output_dict[j], f)

                with open(
                    f"features/{task_ids[j]}/{original_hashs[j]}/metadata.txt",
                    "a",
                    encoding="utf-8",
                ) as f:
                    ...
                    for key in output_dict[j].keys():
                        f.write(
                            f"{key} {type(output_dict[j][key])} {output_dict[j][key].shape}\n"
                        )
...
class Qwen2_5OmniThinkerForConditionalGeneration(
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if os.environ.get("GET_FEATURE", "0") == "1":
            llm_mask = (
                (input_ids != 151655)  # image_token_index
                & (input_ids != 151643) # pad_token_id
                & (input_ids != 151646) # audio_token_index
                & (input_ids != 151656)  # vision_start_token_id
            )
            ...
            for i in range(inputs_embeds.size(0)):
                import pickle
                with open(
                    f"features/{task_ids[i]}/{original_hashs[i]}/embed.pkl",
                    "wb",
                ) as f:
                    output_dict = {}
                    output_dict["llm"] = inputs_embeds[i][llm_mask[i]].detach().cpu()
                    output_dict["image"] = inputs_embeds[i][image_mask[i]].detach().cpu()
                    output_dict["audio"] = inputs_embeds[i][audio_mask[i]].detach().cpu()
                    output_dict["video"] = inputs_embeds[i][video_mask[i]].detach().cpu()
                    output_dict["all"] = inputs_embeds[i].detach().cpu()
                    output_dict["token"] = input_ids[i].detach().cpu()
                    pickle.dump(output_dict, f)
...
class Qwen2_5OmniForConditionalGeneration(Qwen2_5OmniPreTrainedModel, GenerationMixin):
    ...
    def generate(
        ...
        **kwargs,
        ...
        embeds_to_talker = (
            thinker_result.hidden_states[0][0].clone().to(input_ids.device)
        )
        ...
        thinker_token_embeds = [
            token_hidden_states[0].to(input_ids.device)
            for token_hidden_states in processed_thinker_hidden
        ]
        thinker_hidden_states = [
            token_hidden_states[-1].to(input_ids.device)
            for token_hidden_states in processed_thinker_hidden
        ]
        ...
        talker_result = self.talker.generate(
            ...
            **{
                k: (v.to(input_ids.device) if torch.is_tensor(v) else v)
